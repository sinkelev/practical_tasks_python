...
def func_time(func):
    """Декоратор, который подсчитывает и выводит сколько времени выполняется функция"""
    def wrapped(*args):
...
def sort_dict(price):
    ...
    sort_price = sorted(price, key=lambda k: k["цена"])
    print(sort_price[0:2])


def lucky_tickets():
    """Функция, которая подсчитывает количество счастливых шестизначных билетов"""
    i = 0
...
# delete_a('123a456a7a')
# check_num(['8-999-777-1111', '+7 999 333 2222', '+7 999-555-11-11', 'absc'])
